...
local function MAVLinkProcessor()
	...
	-- From: pymavlink/dialects/v10/ardupilotmega.py
    _crc_extra = {}
    _crc_extra[75] = 0x9e
    _crc_extra[76] = 0x98
    _crc_extra[235] = 0xb3
    ...
    _messages[73] = { -- MISSION_ITEM_INT (73)
        {"param1", "<f"}, {"param2", "<f"}, {"param3", "<f"}, {"param4", "<f"},
        {"x", "<i4"}, {"y", "<i4"}, {"z", "<f"}, {"seq", "<I2"},
        {"command", "<I2"}, {"target_system", "<B"}, {"target_component", "<B"},
        {"frame", "<B"}, {"current", "<B"}, {"autocontinue", "<B"}
    }
	_messages[86] = { -- SET_POSITION_TARGET_GLOBAL_INT (86)
		{"time_boot_ms", "<I4"},
		{"lat_int", "<i4>"},
		{"lon_int", "<i4>"},
		{"alt", "<f"},
    ...
    _messages[11] = { -- SET_MODE (11)
        { "custom_mode", "<I4" }, { "target_system", "<B" }, { "base_mode", "<B" },
    }
    _messages[41] = { -- MISSION_SET_CURRENT (41)
    ...
    function self.parseMAVLink(byte)
        ...
        if _mavdecodestate == 3 and #_mavbuffer ==
            ...
            -- only process COMMAND_LONG and COMMAND_INT, MISSION_ITEM_INT and SET_POSITION_TARGET_GLOBAL_INT messages
			...
			elseif _mavresult.msgid == self.SET_POSITION_TARGET_GLOBAL_INT then
				gcs:send_text(6, "***** SET_POSITION_TARGET_GLOBAL_INT received! *****")
				gcs:send_text(6, "New Lat.: " .. _mavresult.lat_int)
				gcs:send_text(6, "New Lon.: " .. _mavresult.lon_int)
				gcs:send_text(6, "New Alt.: " .. _mavresult.alt)
				gcs:send_text(6, "New Coord. Frame: " .. _mavresult.coordinate_frame)
				local loc = Location()
				loc:lat(_mavresult.lat_int)
				loc:lng(_mavresult.lon_int)
				loc:alt(_mavresult.alt)
				if _mavresult.coordinate_frame == 10 then -- MAV_FRAME_GLOBAL_TERRAIN_ALT
					loc:terrain_alt(true)
				elseif _mavresult.coordinate_frame == 3 then -- MAV_FRAME_GLOBAL_RELATIVE_ALT
					loc:relative_alt(true)
				elseif _mavresult.coordinate_frame == 0 then --
					loc:origin_alt(true)
				end
				vehicle:set_target_location(loc)
            ...
            elseif _mavresult.msgid == self.COMMAND_LONG or _mavresult.msgid ==
                ...
                elseif _mavresult.command == 176 then -- MAV_CMD_DO_SET_MODE
					...
					num_attempts = 0
					repeat
						mode_switch_result = vehicle:set_mode(_mavresult.param2)
						num_attempts = num_attempts + 1
					until (mode_switch_result == true or num_attempts >= 5)
...
local function RockblockModem()
    ...
    function self.rxdata(inchar)
        ...
        elseif self.in_read_cycle and #_str_recieved == self.rx_len + 3 then
            ...
            if RCK_DEBUG:get() == 1 then
				...
				gcs:send_text(3, "Rockblock: Modem rx msg: "..
								  self.hexstring(_modem_history[#_modem_history]))
            end
            ...
            if _modem_history[#_modem_history - 2] == 'AT+SBDRB' and
                ...
                elseif checksumrx ~= (tostring(highByte) .. tostring(lowByte)) then
                    ...
                    if RCK_DEBUG:get() == 1 then
                        ...
                        gcs:send_text(3, "Rockblock: Received message: " ..
									      self.hexstring(msg))
                                          --self.nicestring(msg))
                    end
                end
                ...
                if #statusReponse == 7 then
                    if tonumber(statusReponse[2]) < 5 then
                        ...
                        if tonumber(statusReponse[4]) == 1 and self.first_sucessful_mailbox_check then
							...
							gcs:send_text(3, "Packet: " .. _AT_read_rx_buffer)
    ...
    function self.nicestring(instr)
        ...
        for i = 1, #instr do
			...
			if c < 0x20 or c > 0x7E then c = 0x40 end
	...
	function self.hexstring(data)
		...
		elseif type(data) == "table" then
			...
			gcs:send_text(3, "RockblockModem.hexstring() - Unsupported type!")
		end
...
function HLSatcom()
    ...
    while n_bytes > 0 do
        byte_read = port:read()
        pkt = rockblock.rxdata(byte_read)
        -- gcs:send_text(3, "Rockblock: pkt Read ::" .. tostring(read) .. " bytes from port")
        n_bytes = n_bytes - 1
    ...
    if rockblock.modem_detected and gcs:get_high_latency_status() and
        ...
        if vehicle:get_wp_distance_m() ~= nil then
			hl2.target_distance = 1000 -- To temporarily prevent unsigned overflow...
